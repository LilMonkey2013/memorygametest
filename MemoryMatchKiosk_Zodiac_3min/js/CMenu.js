function CMenu(){
    ...
    this._init = function(){
		...
		var _oBgHeader = createBitmap(s_oSpriteLibrary.getSprite('bg_menu_header'));
		_oBgHeader.x = CANVAS_WIDTH/2 - 90;
		_oBgHeader.y = CANVAS_HEIGHT/2 - 484;
		_oBgHeader.scale = 0.5;
        s_oStage.addChild(_oBgHeader);
		
	_pStartPosPlay = {x:(CANVAS_WIDTH/2),y:CANVAS_HEIGHT - 70};
        var oSprite = s_oSpriteLibrary.getSprite('but_menu_bg');

        _oButPlay = new CTextButton(_pStartPosPlay.x - 20,_pStartPosPlay.y,oSprite,TEXT_PLAY,FONT_GAME,"White","24",s_oStage);
	_oButPlay.setScale(2);
        _oButPlay.addEventListener(ON_MOUSE_UP, this._onButPlayRelease, this);
        
        var oSpriteCredits = s_oSpriteLibrary.getSprite('but_credits');
        _pStartPosCredits = {x:oSpriteCredits.width/2 + 15,y:oSpriteCredits.height/2 + 10};
    ...
    this.refreshButtonPos = function(iNewX,iNewY){
        _oButPlay.setPosition(_pStartPosPlay.x + 15,_pStartPosPlay.y - iNewY);
        if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
